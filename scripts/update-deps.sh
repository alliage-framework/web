...
ALLIAGE_VERSION=$(get_minimal_version $(jq -r ".devDependencies[\"$ALLIAGE_PACKAGE_NAME\"]" "$NX_WORKSPACE_ROOT/package.json") "^")
ALLIAGE_CORE_VERSION=$(get_minimal_version $(jq -r ".devDependencies[\"$ALLIAGE_CORE_PACKAGE_NAME\"]" "$NX_WORKSPACE_ROOT/package.json") "^")
